...
def generate_questions(material_path, subject, count=5):
    ...
    try:
        ...
        if subject == "數學":
            math_mental_rules = """
**🔢 數學科特別設計規則（心算友善 — 學生可能喺車上／冇紙筆做題！）：**
1. **數字要細同齊整** — 盡量用一位小數同簡單整數（例如 4.2、0.6、28.4、80）；避免三位以上小數、大數長除法
2. **答案要「靚」** — 設計數字令答案係整數或最多一位／兩位小數（例如 6.2、0.35、42.2）；避免無窮小數（除非題目本身係考「取近似值」）
3. **運算步驟最多 2 步** — 唔好出需要多步長計算嘅題；心算或簡單草稿就做到
