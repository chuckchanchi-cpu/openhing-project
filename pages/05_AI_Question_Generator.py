...
def extract_json_from_text(text):
    ...
    # Find the first [ and last ] to grab the array
    start = text.find("[")
    end = text.rfind("]")
...
def generate_questions(subject, grade, topic, count=5):
    """Generate practice questions using AI with robust JSON parsing."""
    try:
        system_prompt = f"""你係一位經驗豐富嘅小學{subject}科老師，專門為{grade}學生設計練習題目。

請嚴格生成恰好 {count} 條練習題目，唔好多亦唔好少。
...
**輸出格式 (純 JSON Array，唔好有任何其他文字、解釋或 markdown 標記)：**
[{{"id":1,"title":"標題","question":"題目內容","difficulty":"簡單","marks":10,"reference_answer":"答案","tip":"提示"}},{{"id":2,"title":"標題","question":"題目內容","difficulty":"中等","marks":10,"reference_answer":"答案","tip":"提示"}}]
...
注意：只返回 JSON Array，不要任何其他文字。"""
        ...
        if response.status_code == 200:
            ...
            if questions and isinstance(questions, list) and len(questions) >= count:
                ...
                # Got fewer than requested but at least 1
                st.warning(f"⚠️ AI 只生成了 {len(questions)} 條題目（要求 {count} 條），已使用全部結果")
                return questions
            else:
                ...
                return [{
                    ...
                    "question": content[:800] if content else "無內容",
                    "difficulty": "中等",
                    "marks": 10,
